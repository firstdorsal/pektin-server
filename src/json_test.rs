...
fn main() -> PektinResult<()> {
    ...
    let caa = RData::CAA(CAA::new_iodef(true, Url::from_str("http://example.com").unwrap()));
    let ns = RData::NS(name.clone());
    let cname = RData::CNAME(name.clone());
    let ptr = RData::PTR(name.clone());
    let soa = RData::SOA(SOA::new(name.clone(), name.clone(), 202108101, 3600, 7200, 14400, 3600));
    ...
    let dnskey = RData::DNSSEC(DNSSECRData::DNSKEY(DNSKEY::new(true, true, false, Algorithm::ED25519, vec![0, 1, 2, 3])));
    let tlsa = RData::TLSA(TLSA::new(CertUsage::CA, Selector::Full, Matching::Sha256, vec![0, 1, 2, 3]));
    ...
    let rrset = RrSet { rr_type: RecordType::A, rr_set: vec![ResourceRecord { ttl: 300, value: a.clone() }, ResourceRecord { ttl: 3600, value: a }] };
